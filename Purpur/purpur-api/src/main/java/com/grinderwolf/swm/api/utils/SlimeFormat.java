...
public class SlimeFormat {
    ...
    public static byte[] serializeWorld(com.grinderwolf.swm.api.world.SlimeWorld world) {
        List<com.grinderwolf.swm.api.world.SlimeChunk> sortedChunks;
        ...
        extraData.getValue().put("properties", world.getPropertyMap().toCompound());
        ...
        try {
            ...
            int minX = sortedChunks.stream().mapToInt(com.grinderwolf.swm.api.world.SlimeChunk::getX).min().orElse(0);
            int minZ = sortedChunks.stream().mapToInt(com.grinderwolf.swm.api.world.SlimeChunk::getZ).min().orElse(0);
            int maxX = sortedChunks.stream().mapToInt(com.grinderwolf.swm.api.world.SlimeChunk::getX).max().orElse(0);
            int maxZ = sortedChunks.stream().mapToInt(com.grinderwolf.swm.api.world.SlimeChunk::getZ).max().orElse(0);
            ...
            for (com.grinderwolf.swm.api.world.SlimeChunk chunk : sortedChunks) {
            ...
            // Tile Entities
            List<com.flowpowered.nbt.CompoundTag> tileEntitiesList = sortedChunks.stream()
            ...
            // Entities
            List<com.flowpowered.nbt.CompoundTag> entitiesList = sortedChunks.stream()
            ...
            // World Maps
            Collection<com.flowpowered.nbt.CompoundTag> worldMaps = world.getWorldMaps();
            CompoundMap map = new CompoundMap();
            map.put("maps", new ListTag<>("maps", TagType.TAG_COMPOUND, new ArrayList<>(worldMaps)));
    ...
    private static byte[] serializeChunks(List<com.grinderwolf.swm.api.world.SlimeChunk> chunks, byte worldVersion) throws IOException {
        ByteArrayOutputStream outByteStream = new ByteArrayOutputStream(16384);
        DataOutputStream outStream = new DataOutputStream(outByteStream);

        for (com.grinderwolf.swm.api.world.SlimeChunk chunk : chunks) {
            // Height Maps
            ...
            // Chunk sections
            com.grinderwolf.swm.api.world.SlimeChunkSection[] sections = chunk.getSections();
            ...
            for (com.grinderwolf.swm.api.world.SlimeChunkSection section : sections) {
                if (section == null) {
                    continue;
                }
